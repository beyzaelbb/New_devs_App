...
const formatAmount = (amount: string): string => {
    ...
    const [intPart, fracPart = '00'] = (negative ? amount.slice(1) : amount).split('.');

    const grouped = new Intl.NumberFormat(undefined).format(BigInt(intPart || '0'));
    const decimalSeparator =
        new Intl.NumberFormat(undefined)
            .formatToParts(1.1)
            .find((part) => part.type === 'decimal')?.value ?? '.';

    return `${negative ? '-' : ''}${grouped}${decimalSeparator}${fracPart.padEnd(2, '0')}`;
};
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    const hasSubCentPrecision = /^\d*\.\d{2}\d*[1-9]/.test(
        data.total_revenue_exact.replace('-', '')
    );
                ...
                <div className="mt-4 h-6">
                    {hasSubCentPrecision && showRaw && (
                        <div className="flex items-center text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded">
                            ...
                            Sub-cent precision rounded for display (exact: {data.total_revenue_exact})
                        </div>
                    )}
                </div>
            </div>
